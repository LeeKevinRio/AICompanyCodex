...
// Compare semantic data, independent of JSON property order.
...
export function importPlan(current,incoming){const local=migratePlayers(validateStore(structuredClone(current))),remote=migratePlayers(validateStore(structuredClone(incoming)));return ['players','games'].flatMap(kind=>remote[kind].map(item=>{const before=local[kind].find(x=>x.id===item.id);return {kind,id:item.id,before,after:item,status:!before?'new':canonical(before)===canonical(item)?'same':'conflict'};}));}
export function mergeImport(current,incoming,choices={}){const next=migratePlayers(validateStore(structuredClone(current)));for(const e of importPlan(current,incoming)){const key=e.kind+':'+e.id;if(e.status==='same')continue;if(e.status==='conflict'&&!['local','incoming'].includes(choices[key]))throw Error('請先選擇每筆衝突要保留的版本');if(e.status==='conflict'&&choices[key]==='local')continue;const item=structuredClone(e.after),index=next[e.kind].findIndex(x=>x.id===e.id);if(index<0)next[e.kind].push(item);else next[e.kind][index]=item;}if(!next.games.some(g=>g.id===next.activeId&&!g.endedAt))next.activeId=next.games.find(g=>g.id===incoming.activeId&&!g.endedAt)?.id??next.games.find(g=>!g.endedAt)?.id??null;return validateStore(next);}
