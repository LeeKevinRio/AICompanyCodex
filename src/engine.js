...
export function totals(game){return game.hands.reduce((a,h)=>handResult(game,h).delta.map((x,i)=>x+a[i]),[0,0,0,0]);}
...
export function validateStore(data){
 ...
 const ids=new Set();
 for(const g of data.games){validateSettings(g);if(g.playerIds&&(!Array.isArray(g.playerIds)||g.playerIds.length!==4||new Set(g.playerIds).size!==4||g.playerIds.some(id=>!data.players?.some(p=>p.id===id))))throw Error('牌局玩家身分不完整。');if(typeof g.id!=='string'||ids.has(g.id)||!Array.isArray(g.hands)||g.hands.length>10000||!Number.isFinite(Date.parse(g.createdAt))||(g.endedAt!==null&&!Number.isFinite(Date.parse(g.endedAt))))throw Error('牌局資料不完整。');ids.add(g.id);for(const h of g.hands){handResult(g,h);if(typeof h.note!=='string'||h.note.length>80)throw Error('紀錄備註格式錯誤。');}}
...
export function playerTotals(data,id,period=''){
 const result={games:0,hands:0,resolved:0,wins:0,self:0,dealIn:0,tai:0,fee:0,net:0,maxTai:null,maxWin:0,maxLoss:0,idle:0,active:0,sumSq:0,history:[]};
 for(const g of data.games){if(period&&!taipeiDate(g.createdAt).startsWith(period))continue;const i=g.playerIds?.indexOf(id)??-1;if(i<0)continue;const a=analytics(g),r=a.rows[i];result.games++;result.hands+=g.hands.length;result.resolved+=a.resolved;for(const k of ['wins','self','dealIn','tai','fee','net'])result[k]+=r[k];result.maxTai=r.maxTai===null?result.maxTai:Math.max(result.maxTai??0,r.maxTai);result.maxWin=Math.max(result.maxWin,r.maxWin);result.maxLoss=Math.max(result.maxLoss,r.maxLoss);result.idle+=r.idle;result.active+=r.active;result.sumSq+=r.sumSq;result.history.push({id:g.id,title:g.title,createdAt:g.createdAt,net:r.net,hands:g.hands.length});}
 return {...result,idleRate:result.hands?result.idle/result.hands:null,selfShare:result.wins?result.self/result.wins:null,volatility:result.hands?Math.sqrt(Math.max(0,result.sumSq/result.hands-(result.net/result.hands)**2)):null,winRate:result.resolved?result.wins/result.resolved:null,selfRate:result.resolved?result.self/result.resolved:null,dealInRate:result.resolved?result.dealIn/result.resolved:null};
}
